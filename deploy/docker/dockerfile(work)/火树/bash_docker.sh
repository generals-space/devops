...
function docker-map() {
	...
	iptables -t nat -I DOCKER -p tcp -m tcp --dport $host_port -j DNAT --to $container_ip:$container_port
	## 将上面的请求包转发出去, 目标就是$container_ip:$container_port, 并添加MASQUERADE标记
	iptables -t nat -I POSTROUTING -p tcp -m tcp --src $container_ip --dst $container_ip --dport $container_port -j MASQUERADE
	## filter表上的DOCKER链被合并入INPUT链. 打开目标容器指定端口的INPUT链上的ACCEPT规则, 接受外部连接
	iptables -t filter -I DOCKER -p tcp -m tcp --dst $container_ip --dport $container_port -j ACCEPT
}
